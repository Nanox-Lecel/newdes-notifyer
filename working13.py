...
class LiveLogWindow:
    ...
    def create_window(self):
        ...
        # Export button
        export_button = ttk.Button(header_frame, text="📊 Export Log (Excel)", 
                                  command=self.export_logs,
                                  style='Primary.TButton')
        export_button.pack(side=tk.RIGHT, padx=(10, 0))
        
        # Date range selection for export
        date_frame = ttk.Frame(header_frame, style='Modern.TFrame')
        ...
        start_date_entry = ttk.Entry(date_frame, textvariable=self.start_date_var, width=10)
        ...
        end_date_entry = ttk.Entry(date_frame, textvariable=self.end_date_var, width=10)
    ...
    def export_logs(self):
        """Export logs to Excel for the specified date range"""
        try:
            ...
            logs = self.log_manager.get_logs_for_export(start_date, end_date)
            ...
            for log_entry in logs:
                ...
                if "] " in log_entry:
                    ...
                    timestamp_str = timestamp_str[1:]  # Remove opening bracket
                    ...
                    try:
                        ...
                        temp_data = {}
                        if "Average Temperature:" in message:
                            temp_data["Type"] = "Average"
                            temp_data["Value"] = float(message.split("Average Temperature: ")[1].split("°C")[0])
                        elif "Max Temperature:" in message:
                            temp_data["Type"] = "Maximum"
                            temp_data["Value"] = float(message.split("Max Temperature: ")[1].split("°C")[0])
                        elif "Storage temperatures:" in message:
                            temp_data["Type"] = "Storage Details"
                            temp_data["Value"] = message
                        elif "Alert:" in message:
                            temp_data["Type"] = "Alert"
                            temp_data["Value"] = message
                        else:
                            temp_data["Type"] = "Info"
                            temp_data["Value"] = message
            ...
            if not df_data:
                messagebox.showinfo("No Data", "No valid temperature data found for export")
                return
            ...
            # Generate filename
            filename = f"temperature_logs_{start_date_str}_to_{end_date_str}.xlsx"
            
            # Save to Excel
            df.to_excel(filename, index=False, sheet_name="Temperature Logs")
...
class LogManager:
    """Manages persistent logging of temperature data"""
    def __init__(self):
        self.log_file = "temperature_logs.csv"
        self.log_buffer = []
    ...
    def setup_logging(self):
        ...
        if not os.path.exists(self.log_file):
            with open(self.log_file, 'w', newline='', encoding='utf-8') as f:
                writer = csv.writer(f)
                writer.writerow(['timestamp', 'type', 'value', 'message'])
        
        # Load existing log count
        try:
            with open(self.log_file, 'r', encoding='utf-8') as f:
                self.last_log_index = sum(1 for _ in f) - 1  # Subtract header
        except:
            self.last_log_index = 0
    ...
    def log_temperature(self, temp_type, value, message=""):
        ...
        # Create log entry
        log_entry = f"[{timestamp}] {message}" if message else f"[{timestamp}] {temp_type}: {value}°C"
        ...
        threading.Thread(target=self._write_to_file, 
                        args=(timestamp, temp_type, value, message),
                        daemon=True).start()
        
        print(log_entry)  # Also print to console
    
    def _write_to_file(self, timestamp, temp_type, value, message):
        """Write log entry to file in a separate thread"""
        try:
            with open(self.log_file, 'a', newline='', encoding='utf-8') as f:
                writer = csv.writer(f)
                writer.writerow([timestamp, temp_type, value, message])
        except Exception as e:
    ...
    def get_all_logs(self):
        ...
        try:
            with open(self.log_file, 'r', encoding='utf-8') as f:
                ...
                for row in reader:
                    if row['message']:
                        logs.append(f"[{row['timestamp']}] {row['message']}")
                    else:
                        logs.append(f"[{row['timestamp']}] {row['type']}: {row['value']}°C")
        except Exception as e:
    ...
    def get_logs_for_export(self, start_date, end_date):
        """Get logs for a specific date range for export"""
        logs = []
        try:
            with open(self.log_file, 'r', encoding='utf-8') as f:
                reader = csv.DictReader(f)
                for row in reader:
                    log_date = datetime.datetime.strptime(row['timestamp'][:10], "%Y-%m-%d").date()
            ...
            print(f"Error reading logs for export: {e}")
...
def main():
    ...
    try:
        ...
        # Check for pandas (required for Excel export)
        try:
            import pandas as pd
            print("✅ Pandas support available")
        except ImportError:
            print("❌ Pandas not available - install with: pip install pandas openpyxl")
            messagebox.showerror("Missing Dependency", "Pandas is required for Excel export.\n\nPlease install it with: pip install pandas openpyxl")
            return
            
    except ImportError as e:
        print(f"Missing dependency: {e}")
        print("Please install required packages:")
        print("pip install psutil plyer matplotlib wmi pandas openpyxl")
        messagebox.showerror("Missing Dependencies", f"Missing required packages:\n\nPlease install: pip install psutil plyer matplotlib wmi pandas openpyxl")
        return
