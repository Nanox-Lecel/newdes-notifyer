...
class TemperatureMonitor:
    ...
    def setup_modern_styles(self):
        """Configure modern styling with custom colors"""
        style = ttk.Style()
        ...
        style.configure('Card.TLabelframe', background=self.colors['surface'], relief='flat', borderwidth=1)
        style.configure('Card.TLabelframe.Label', background=self.colors['surface'], foreground=self.colors['primary'])
    ...
    def setup_ui(self):
        ...
        title_label = ttk.Label(header_frame, text="Storage Temperature Monitor", 
        ...
        note_label = ttk.Label(header_frame, 
                              text="Note: All temperatures shown are based on room temperature not the actual Device", 
                              background=self.colors['background'],
                              foreground=self.colors['secondary'],
                              font=("Segoe UI", 9))
        note_label.pack(anchor=tk.W, pady=(5, 0))
        
        # Status bar with modern cards
        ...
        sensor_status_label = ttk.Label(status_card, textvariable=self.sensor_status_var,
                                      ...
                                      font=("Segoe UI", 9))
        sensor_status_label.pack(anchor=tk.W)
        ...
        ttk.Label(avg_card, text="Average Storage Temp", 
                 background=self.colors['surface'],
                 foreground=self.colors['text_secondary'],
        ...
        # Status card
        status_indicator_card = ttk.Frame(metrics_frame, style='Card.TFrame', padding="20")
        status_indicator_card.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(10, 0))
        
        ttk.Label(status_indicator_card, text="System Status", 
                 background=self.colors['surface'],
                 foreground=self.colors['text_secondary'],
                 font=("Segoe UI", 11)).pack(anchor=tk.W)
        
        self.status_indicator = tk.Canvas(status_indicator_card, width=24, height=24, bg="gray", 
                                         highlightthickness=0, relief='flat')
        self.status_indicator.pack(anchor=tk.W, pady=(5, 0))
        
        self.status_var = tk.StringVar(value="Status: Initializing...")
        status_label = ttk.Label(status_indicator_card, textvariable=self.status_var,
                                background=self.colors['surface'],
        ...
        # Storage temperatures display frame with modern card
        storage_frame = ttk.LabelFrame(left_column, text="Storage Device Temperatures", 
                                      style='Card.TLabelframe', padding="15")
        storage_frame.pack(fill=tk.BOTH, expand=True, pady=(0, 15))
        
        # Create scrollable frame for storage devices
        storage_canvas = tk.Canvas(storage_frame, height=150, bg=self.colors['surface'], highlightthickness=0)
        scrollbar = ttk.Scrollbar(storage_frame, orient="vertical", command=storage_canvas.yview)
        self.scrollable_storage_frame = ttk.Frame(storage_canvas, style='Card.TFrame')
        ...
        # Controls frame with modern card
        controls_frame = ttk.LabelFrame(left_column, text="Monitoring Controls", 
                                       style='Card.TLabelframe', padding="15")
        ...
        self.start_button = ttk.Button(button_frame, text="▶ Start Alert Monitoring", 
                                      command=self.start_alert_monitoring)
        self.start_button.pack(side=tk.LEFT, padx=(0, 10))
        
        self.stop_button = ttk.Button(button_frame, text="⏹ Stop Alert Monitoring", 
        ...
        ttk.Label(refresh_frame, text="Update every:").pack(side=tk.LEFT)
        ...
        refresh_combo.pack(side=tk.LEFT, padx=(5, 5))
        ttk.Label(refresh_frame, text="seconds").pack(side=tk.LEFT, padx=(0, 10))
        ...
        ttk.Button(utils_frame, text="📊 Sensor Info", 
                  command=self.show_sensor_info).pack(side=tk.LEFT, padx=(0, 10))
        
        ttk.Button(utils_frame, text="✉️ Test Email", 
                  command=self.send_test_email).pack(side=tk.LEFT)
        
        # Settings frame with modern card
        settings_frame = ttk.LabelFrame(left_column, text="Temperature Settings (°C) - Adjusted Values", 
                                       style='Card.TLabelframe', padding="15")
        ...
        ttk.Label(settings_grid, text="Warning Temp:").grid(row=0, column=0, sticky=tk.W, padx=(0, 10))
        self.warning_var = tk.StringVar(value=str(self.warning_temp))
        warning_entry = ttk.Entry(settings_grid, textvariable=self.warning_var, width=8)
        warning_entry.grid(row=0, column=1, padx=(0, 20))
        
        # Critical temperature
        ttk.Label(settings_grid, text="Critical Temp:").grid(row=0, column=2, sticky=tk.W, padx=(0, 10))
        self.critical_var = tk.StringVar(value=str(self.critical_temp))
        critical_entry = ttk.Entry(settings_grid, textvariable=self.critical_var, width=8)
        critical_entry.grid(row=0, column=3, padx=(0, 20))
        
        # Update settings button
        ttk.Button(settings_grid, text="💾 Update Settings", 
                  command=self.update_settings).grid(row=0, column=4)
        
        # Right column - Graph
        right_column = ttk.Frame(content_frame, style='Modern.TFrame')
        right_column.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True)
        
        # Temperature graph with modern card
        graph_frame = ttk.LabelFrame(right_column, text="Max Temperature History - Adjusted (Last 60 minutes)", 
                                    style='Card.TLabelframe', padding="15")
        graph_frame.pack(fill=tk.BOTH, expand=True)
        
        # Create modern matplotlib figure
        plt.rcParams['axes.facecolor'] = self.colors['surface']
        plt.rcParams['figure.facecolor'] = self.colors['surface']
        ...
        self.next_email_var = tk.StringVar(value="Next email report: --")
    ...
    def update_time_display(self):
        ...
        if time_until_next > 0:
            ...
            self.next_email_var.set(f"Next email report: {minutes:02d}:{seconds:02d}")
        else:
            self.next_email_var.set("Next email report: Soon")
    ...
    def update_status_indicator(self, temperature):
        ...
        self.status_indicator.create_oval(2, 2, 22, 22, fill=color, outline=self.colors['border'], width=2)
...
"""
    ...
    def update_storage_display(self):
        ...
        if self.storage_temperatures:
            ...
            for device_name, temp in self.storage_temperatures.items():
                # Device card
                device_card = ttk.Frame(self.scrollable_storage_frame, style='Card.TFrame', padding="10")
                device_card.pack(fill=tk.X, pady=(0, 5))
                
                # Device label
                device_label = ttk.Label(device_card, text=f"{device_name}:", 
                                       background=self.colors['surface'],
                                       font=("Segoe UI", 9))
                device_label.pack(side=tk.LEFT, padx=(5, 5))
                
                # Temperature value with color coding
                temp_color = self.colors['text_primary']
                if temp >= self.critical_temp:
                    temp_color = self.colors['error']
                elif temp >= self.warning_temp:
                    temp_color = self.colors['warning']
                    
                temp_label = ttk.Label(device_card, text=f"{temp:.1f} °C", 
                                     background=self.colors['surface'],
                                     foreground=temp_color,
                                     font=("Segoe UI", 9, "bold"))
                temp_label.pack(side=tk.RIGHT, padx=(5, 5))
    ...
    def update_graph(self):
        ...
        if len(self.temp_history) > 0:
            ...
            # Modern plot styling
            self.ax.plot(time_minutes, list(self.temp_history), color=self.colors['primary'], linewidth=2, label='Max Storage Temperature')
            self.ax.axhline(y=self.warning_temp, color=self.colors['warning'], linestyle='--', alpha=0.7, label=f'Warning ({self.warning_temp}°C)')
            self.ax.axhline(y=self.critical_temp, color=self.colors['error'], linestyle='--', alpha=0.7, label=f'Critical ({self.critical_temp}°C)')
