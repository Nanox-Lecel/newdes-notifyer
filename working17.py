...
class DateSearchWindow:
    """Modal window for date range search in logs"""
    def __init__(self, parent, log_manager, live_log_window):
        self.parent = parent
        self.log_manager = log_manager
        self.live_log_window = live_log_window
        self.window = None
        self.create_window()
        
    def create_window(self):
        """Create the date search modal window"""
        self.window = tk.Toplevel(self.parent)
        self.window.title("Search Logs by Date Range")
        ...
        # Title
        title_label = ttk.Label(main_frame, text="Search Logs by Date Range", 
                               background='#0f172a',
                               foreground='#f8fafc',
        ...
        date_frame = ttk.Frame(main_frame, style='Modern.TFrame')
        ...
        ttk.Label(start_frame, text="Start Date (YYYY-MM-DD):", 
                 background='#0f172a',
                 foreground='#cbd5e1').pack(side=tk.LEFT)
        ...
        start_date_entry.pack(side=tk.RIGHT)
        ...
        end_date_entry.pack(side=tk.RIGHT)
    ...
    def search_logs(self):
        """Search and display logs for the selected date range"""
        try:
            ...
            logs = self.log_manager.get_logs_for_date_range(start_date, end_date)
            
            if not logs:
                messagebox.showinfo("No Data", "No logs found for the specified date range")
                return
            
            # Update the live log window with the searched logs
            self.live_log_window.display_searched_logs(logs, start_date_str, end_date_str)
...
class LiveLogWindow:
    """Live Log window for displaying and exporting temperature logs"""
    def __init__(self, parent, log_manager):
        self.parent = parent
        self.log_manager = log_manager
        self.window = None
        self.is_running = True
        self.current_logs = []  # Store current displayed logs for export
    ...
    def create_window(self):
        ...
        # Search button
        search_button = ttk.Button(button_frame, text="🔍 Search Date Range", 
                                  command=self.show_search_modal,
                                  style='Secondary.TButton')
        search_button.pack(side=tk.RIGHT, padx=(10, 0))
        
        # Export button
        export_button = ttk.Button(button_frame, text="📊 Export Log (.logs)", 
                                  command=self.export_logs,
                                  style='Primary.TButton')
        export_button.pack(side=tk.RIGHT, padx=(10, 0))
        
        # Date range display
        self.date_range_var = tk.StringVar(value="Showing: All Logs")
        date_range_label = ttk.Label(header_frame, textvariable=self.date_range_var,
                                    background='#0f172a',
                                    foreground='#cbd5e1',
                                    font=("Segoe UI", 9))
        date_range_label.pack(side=tk.LEFT, padx=(20, 0))
        
        # Date range selection bar - ADDED BACK
        date_bar_frame = ttk.Frame(self.window, style='Modern.TFrame')
        date_bar_frame.pack(fill=tk.X, padx=15, pady=(0, 10))
        
        ttk.Label(date_bar_frame, text="From:", 
                 background='#0f172a',
                 foreground='#cbd5e1').pack(side=tk.LEFT)
        
        self.start_date_var = tk.StringVar(value=datetime.datetime.now().strftime("%Y-%m-%d"))
        start_date_entry = ttk.Entry(date_bar_frame, textvariable=self.start_date_var, width=12)
        start_date_entry.pack(side=tk.LEFT, padx=(5, 15))
        
        ttk.Label(date_bar_frame, text="To:", 
                 background='#0f172a',
                 foreground='#cbd5e1').pack(side=tk.LEFT)
        
        self.end_date_var = tk.StringVar(value=datetime.datetime.now().strftime("%Y-%m-%d"))
        end_date_entry = ttk.Entry(date_bar_frame, textvariable=self.end_date_var, width=12)
        end_date_entry.pack(side=tk.LEFT, padx=(5, 10))
        
        # Search button in date bar
        date_search_button = ttk.Button(date_bar_frame, text="Search", 
                                       command=self.search_from_date_bar,
                                       style='Secondary.TButton')
        date_search_button.pack(side=tk.LEFT, padx=(5, 0))
    ...
    def show_search_modal(self):
        """Show the date search modal window"""
        DateSearchWindow(self.window, self.log_manager, self)
    
    def search_from_date_bar(self):
        """Search logs using the date range from the date bar"""
        try:
            start_date_str = self.start_date_var.get()
            end_date_str = self.end_date_var.get()
            ...
            # Update the display with searched logs
            self.display_searched_logs(logs, start_date_str, end_date_str)
            
        except Exception as e:
            messagebox.showerror("Search Error", f"Failed to search logs: {str(e)}")
    
    def display_searched_logs(self, logs, start_date, end_date):
        """Display searched logs in the log text area"""
        self.log_text.delete(1.0, tk.END)
        self.current_logs = logs  # Store for export
        ...
        if logs:
            ...
            if start_date == end_date:
                self.date_range_var.set(f"Showing: {start_date}")
            else:
                self.date_range_var.set(f"Showing: {start_date} to {end_date}")
        else:
    ...
    def refresh_log_display(self):
        ...
        if logs:
            ...
            self.date_range_var.set("Showing: All Logs (Live)")
        else:
    ...
    def update_live_log(self):
        ...
        if self.is_running and self.window.winfo_exists():
            # Only update if we're showing all logs (not in search mode)
            if "All Logs" in self.date_range_var.get():
                ...
                if new_logs:
                    for log_entry in new_logs:
                        self.log_text.insert(tk.END, log_entry + "\n")
                        self.current_logs.append(log_entry)  # Update stored logs
    ...
    def export_logs(self):
        """Export logs to .logs file for the current display (search results or all logs)"""
        try:
            # Determine what to export based on current display
            if self.current_start_date and self.current_end_date:
                # Export searched date range
                start_date = datetime.datetime.strptime(self.current_start_date, "%Y-%m-%d").date()
                end_date = datetime.datetime.strptime(self.current_end_date, "%Y-%m-%d").date()
            else:
                # Export all logs (last 30 days by default)
                start_date = datetime.datetime.now().date() - datetime.timedelta(days=30)
                end_date = datetime.datetime.now().date()
            
            # Export logs using LogManager
            success = self.log_manager.export_logs_to_file(start_date, end_date, self.current_logs)
            
            if success:
                messagebox.showinfo("Export Successful", 
                                  "Logs exported successfully to Downloads folder!")
            else:
                messagebox.showinfo("No Data", "No logs found for export")
...
class LogManager:
    ...
    def export_logs_to_file(self, start_date, end_date, specific_logs=None):
        """Export logs to .logs file in Downloads folder only"""
        # Use specific logs if provided (from search results), otherwise get from date range
        if specific_logs is not None:
            logs = specific_logs
        else:
        ...
        try:
            ...
            # Condition 2: Export only to Downloads folder
